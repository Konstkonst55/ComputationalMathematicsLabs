...
namespace _6
{
    public class NewtonBackwardInterpolation : NewtonInterpolation
    {
        ...
        public double Compute(double x, bool useWrite = true)
        {
            ...
            double result = _differenceTable[n - 1, 0];
            double term = 1;

            for (int i = 1; i < n; i++)
            {
                term *= (q + (i - 1)) / i;
                double addTerm = _differenceTable[n - i - 1, i] * term;
                result += addTerm;

                if (useWrite) Console.WriteLine($"Шаг {i}: {_differenceTable[n - i - 1, i]:F6} / {Factorial(i)} * {term:F6} = {addTerm:F6}");
            }
